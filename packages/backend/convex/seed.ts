  ...
  },
  {
    ...
    statements: [
      ...
      },
      {
        ...
        fasit: "usant" as const,
        explanation: "Det er omvendt: britene sier 'lorry' og amerikanerne sier 'truck'.",
      },
      {
      ...
      },
      {
        ...
        fasit: "usant" as const,
        explanation: "Grammatikksystemene er i hovedsak like. Forskjellene er små.",
      },
      {
...
export const seed = mutation({
  args: {},
  handler: async (ctx) => {
    const existing = await ctx.db.query("fagprats").first();
    if (existing) {
    ...
    return { seeded: true, message: `Seeded ${SEED_DATA.length} FagPrater.` };
  },
});
