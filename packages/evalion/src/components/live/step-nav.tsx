...
export function StepNav({ currentStep, completedSteps = [], onStepClick }: StepNavProps) {
  ...
  return (
    <nav className="fixed right-0 bottom-0 left-0 z-40 flex h-[100px] min-h-[100px] items-center justify-center border-t-[1.5px] border-[var(--color-divider-soft)] bg-white px-6">
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  isActive && "text-[var(--color-purple-700)]",
                  isCompleted && "text-[var(--color-text-ink-soft)]",
                  isInactive && "text-[var(--color-text-ink-faint)]",
                )}
              >
                {step.label}
