...
var FAGPRAT_DATA = {
  ...
  "brok-prosent": {
    ...
    subject: "Naturfag / Samfunnsfag",
    level: "10. trinn",
    type: "oppsummering",
    concepts: ["Klimaendringer", "B\u00e6rekraft", "FNs b\u00e6rekraftsm\u00e5l", "Energi"],
    statements: [
      {
        text: "Klimaendringene rammer fattige land hardest.",
        fasit: "sant",
        color: "blue",
        explanation: "Fattige land har mindre penger til \u00e5 beskytte seg mot flom, t\u00f8rke og ekstremv\u00e6r, og mangler ofte gode nok bygninger, veier og helsevesen til \u00e5 h\u00e5ndtere konsekvensene. Mange av disse landene ligger ogs\u00e5 i <span class=\"explanation-highlight\">omr\u00e5der n\u00e6r ekvator</span> som rammes hardest av temperatur\u00f8kning, og befolkningen er ofte avhengig av jordbruk som er sv\u00e6rt s\u00e5rbart for klimaendringer.",
        begrunnelse: "\u00abFattige land har ikke r\u00e5d til \u00e5 beskytte seg, og mange ligger der det blir varmest\u00bb"
      },
      {
        text: "El-biler er helt utslippsfrie.",
        fasit: "usant",
        color: "orange",
        explanation: "El-biler slipper ikke ut avgasser n\u00e5r de kj\u00f8rer, men det krever mye energi og ressurser \u00e5 lage batteriene, noe som f\u00f8rer til utslipp og natur\u00f8deleggelse. I tillegg avhenger det totale klimaavtrykket av <span class=\"explanation-highlight\">hvordan str\u00f8mmen de lades med er produsert</span> \u2013 en el-bil ladet p\u00e5 kullkraft er langt fra utslippsfri, mens en ladet p\u00e5 norsk vannkraft er mye bedre.",
        begrunnelse: "\u00abDe forurenser ikke n\u00e5r de kj\u00f8rer, men batteriene m\u00e5 lages og str\u00f8mmen m\u00e5 komme fra et sted\u00bb"
      },
      {
        text: "Alle FNs b\u00e6rekraftsm\u00e5l handler om klima og milj\u00f8.",
        fasit: "usant",
        color: "purple",
        explanation: "Bare noen f\u00e5 av de 17 b\u00e6rekraftsm\u00e5lene handler direkte om klima og milj\u00f8, som <span class=\"explanation-highlight\">m\u00e5l 13</span> (stoppe klimaendringene) og m\u00e5l 14 og 15 (livet i havet og p\u00e5 land). Resten handler om ting som fattigdom, helse, utdanning, likestilling og fred.",
        begrunnelse: "\u00abNoen av m\u00e5lene handler om klima, men de fleste handler om helt andre ting som helse og utdanning\u00bb"
      },
      {
        text: "Fornybar energi kan erstatte fossil energi og l\u00f8se klimaproblemet.",
        fasit: "delvis",
        color: "yellow",
        explanation: "Fornybar energi som sol og vind kan i teorien erstatte fossil energi, men per i dag dekker fornybare kilder bare rundt <span class=\"explanation-highlight\">30 % av verdens energibehov</span>, og vi mangler gode nok l\u00f8sninger for lagring og stabil str\u00f8mforsyning. I tillegg handler klimaproblemet om mer enn bare str\u00f8m \u2013 vi m\u00e5 ogs\u00e5 endre industri, transport, matproduksjon og forbruksm\u00f8nster.",
        begrunnelse: "\u00abSol og vind kan hjelpe mye, men det er ikke nok alene \u2013 vi m\u00e5 endre mye mer\u00bb"
      }
    ]
  },
...
// ── Demo isolation: when loaded inside an iframe (demo), always use
//    the default FagPrat and statement index 0 so that external
//    localStorage changes from standalone sessions never leak in. ──
var _isDemoEmbed = (window.parent !== window);

// ── Helper: get current FagPrat from localStorage ──
function getCurrentFagPrat() {
  if (_isDemoEmbed) return getFagPrat('brok-prosent');
  var id = localStorage.getItem('fagprat-current-id') || 'brok-prosent';
  return getFagPrat(id);
}
...
function getCurrentStatement() {
  ...
  if (_isDemoEmbed) {
    var demoIdx = parseInt(localStorage.getItem('fagprat-demo-statement') || '0');
    return fp && fp.statements[demoIdx] ? fp.statements[demoIdx] : null;
  }
...
function getCurrentStatementIndex() {
  if (_isDemoEmbed) return parseInt(localStorage.getItem('fagprat-demo-statement') || '0');
  return parseInt(localStorage.getItem('fagprat-current-statement') || '0');
}

// ── When embedded in iframe (demo), notify parent and hide demo-irrelevant UI ──
if (window.parent !== window) {
  try {
    var pageMatch = location.href.match(/fagprat-steg(\d+)/);
    if (pageMatch) {
      window.parent.postMessage({ type: 'fagprat-nav', steg: parseInt(pageMatch[1]) }, '*');
    }
  } catch (e) {}
  // Hide nav, record button and end-activity button in demo context
  var _demoStyle = document.createElement('style');
  _demoStyle.textContent = '.step-nav-bar{display:none !important;}.btn-end-activity{display:none !important;}.btn-record-topbar{display:none !important;}.btn-next-step{display:none !important;}';
